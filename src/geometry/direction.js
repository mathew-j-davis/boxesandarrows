...
class Direction {
    ...
    /**
     * Get a standard TikZ anchor name or null if not a valid anchor
     * 
     * @param {string} anchor - The anchor name to validate
     * @returns {string|null} - Standard TikZ anchor name (e.g., "north east") or null if not recognized
     */
    static getStrictAnchorName(anchor) {
        ...
        // If we couldn't find a vector, this isn't a standard anchor
        if (!vector) {
            return null;
        }
        const standardName = this.getDirectionNameFromVector(vector);

        return standardName;
    }

    /**
     * Get a standard TikZ anchor name or null if not a basic anchor
     * 
     ...
     * This is useful when:
     * - we need to convert shorthand to a standard form
     * - we need to convert to a vector for geometry calculations
     * @param {string} anchor - The anchor name to validate
     * @returns {string|null, Point2D} - Standard TikZ anchor name (e.g., "north east") or null if not recognized
     */
        static getStrictAnchorNameAndVector(anchor) {
            if (!anchor) {
                return null; //{standard: null, vector: null};
            }
            ...
            if (!vector) {
                return {standard: null, vector: null};
            }
            ...
            return {standard: canonicalName, vector: vector};
        }
}
