...
class Direction {
     ...
     * 
     * @param {string} anchor - Anchor name to convert (e.g., "ne", "north east", "right")
     * @returns {string|null, Point2D} - Canonical TikZ anchor name (e.g., "north east") or null if not recognized
     */
        static getStrictAnchorNameAndVector(anchor) {
            ...
            // If we couldn't find a vector, this isn't a recognized anchor
            if (!vector) {
                return {anchor: null, vector: null};
            }
            ...
            return {canonical: canonicalName, vector: vector};
        }
}
