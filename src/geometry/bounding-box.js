...
class BoundingBox {
    ...
    static fromNode(node) {
        ...
        try {
            const width = node.dimensions.widthScaled || 0;
            const height = node.dimensions.heightScaled || 0;
            ...
            const left = node.position.xScaled - halfWidth - (anchorVector.x * halfWidth);
            const right = left + width;
            const bottom = node.position.yScaled - halfHeight - (anchorVector.y * halfHeight);
