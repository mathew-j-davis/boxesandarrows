    ...
    preDrawNode(node) {
        ...
        if (node.anchor) {
            ...
            if (standardizedAnchor) {
                node_tikz['anchor'] = standardizedAnchor;
            }
        }
        
        // Apply colors if specified
        if (node.fillcolor) {
            node_tikz['fill'] = this.styleHandler.registerColor(node.fillcolor);
        }
        if (node.edge_color) {
            node_tikz['draw'] = this.styleHandler.registerColor(node.edge_color);
        }
        if (node.textcolor) {
            node_tikz['text'] = this.styleHandler.registerColor(node.textcolor);
        }
        ...
        if (node.tikz_object_attributes) {
            ...
            if (processedAttributes.tikz) {
                Object.assign(node_tikz, processedAttributes.tikz);
            }
        }
        
        // NEW STEP: Process all hex colors in tikzAttributes
        for (const [key, value] of Object.entries(node_tikz)) {
            if (typeof value === 'string' && value.startsWith('#')) {
                node_tikz[key] = this.styleHandler.registerColor(value);
            }
        }
        
        // 5. Generate TikZ style string from our attributes
        let styleStr = this.generateTikzOptions(node_tikz);
...
`;
    ...
    drawGrid(gridSpacing) {
        ...
        for (let x = minX; x <= maxX; x += scaledGridSpacingX) {
            // Draw vertical line
            gridContent.push(`\\draw[${gridStyle}] (${x},${minY}) -- (${x},${maxY});`);
        ...
        for (let y = minY; y <= maxY; y += scaledGridSpacingY) {
            ...
            gridContent.push(`\\draw[${gridStyle}] (${minX},${y}) -- (${maxX},${y});`);
    ...
    drawNodesAndEdges(nodes, edges) {
        ...
        for (const node of nodes) {
            if (node?.rendered_output) {
                content.push(node.rendered_output);
            }
        }
        ...
        for (const edge of edges) {
            if (edge.rendered_output) {
                content.push(edge.rendered_output);
            }
        }
